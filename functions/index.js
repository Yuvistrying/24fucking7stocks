...
async function updateAllStockData() {
  ...
  const tickersSnapshot = await db.ref('tickers').once('value');
  const tickers = tickersSnapshot.val() || {};
  
  // List of all unique tickers
  const allTickers = [...new Set(Object.values(tickers).flat())];
...
// Add a ticker
app.post('/api/add_ticker', async (req, res) => {
  try {
...
// Remove a ticker
app.post('/api/remove_ticker', async (req, res) => {
  try {
    const { ticker, userId } = req.body;
...
// Schedule the stock data update
